...
namespace DnevnaDoza.Controllers
{
    public class PrikazController : Controller
    {
        ...
        public PrikazController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        // Za prikaz kategorija
      

        public IActionResult Index(KategorijeProizvoda? kategorija)
        {
            var proizvodi = _context.Proizvod.AsQueryable();

            if (kategorija.HasValue)
            {
                proizvodi = proizvodi.Where(p => p.Kategorija == kategorija.Value);
            }
